...
{
    public:
        Array(); // constructor with no input
        Array(int); //Copy Constructor with size for sorted i values in for loop
        virtual ~Array(); //  Deconstructor

        T *arr; // Pointer to array of type T
        int sizeOfArr; // the size of the array
        void displayArr(); // displays the content of the array seperated by commas
        void orderArr(int); // makes an array in sorted ascending order
        void randArr(int); // makes an array in random order
        void inverseArr(int); // makes an array in reverse order
        void almostOrder(int); // makes an array 10% shuffled
        void swapping(T*,int,int); // swaps the values in the array
        void insertionSort(); // sorts the array using Insertion Sort Implementation
        void selectionSort(); // sorts the array using Selection Sort Implementation
        void bubbleSort(); // sorts the array using Bubble Sort Implementation
        T partition(T *,int ,int); // Partitions the array and returns a pivot point in the array
        T quickSort(T*,int,int); // Sorts the array using Quick Sort Implementation
        void merge(T*, int, int , int , int); // merges the divided array elements back into one array
        void mergeSort(T*, int, int); // Sorts the array using Merge Sort Implementation
};
